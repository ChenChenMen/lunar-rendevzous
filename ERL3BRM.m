function [A,B] = ERL3BRM(ref_r,targ_orb)
% Exact Restricted Linearized 3 Body Relative Motion
...
% 1:3 - r, position vecotor of target in MCO
% 4:6 - rdot, velocity vecotor of target in MCO
r = ref_r(1:3); rdot = ref_r(4:6);
...
% % % Input matrix % % %
B = [zeros(3); eye(3)]*targ_orb.thrust;
...
% OMEGAli, skew-symmetric matrix for omegali
OMEGAli = skew(omegali);
OMEGAlidot = skew(omegalidot);

% % % State matrix % % %
Aqdotq = -OMEGAlidot-OMEGAli*OMEGAli-mu/rn^3*(eye(3)-3*(r*r')/rn^2)-(1-mu)/ren^3*(eye(3)-3*(re*re')/ren^2);
A = [zeros(3) eye(3); Aqdotq -2*OMEGAli];
